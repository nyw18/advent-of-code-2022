...
def count_folders(dir):
    ...
    while cur_dir := to_check.pop():
        for subdir in cur_dir.values():
            if isinstance(subdir, int):
                continue
            cur_size = get_dir_size(subdir)
            if get_dir_size(subdir) > (get_dir_size(dir_tree) - 40000000):
                if cur_size < smallest_freeable:
                    smallest_freeable = cur_size
                to_check.append(subdir)
        if len(to_check) == 0:
            break
    return smallest_freeable
